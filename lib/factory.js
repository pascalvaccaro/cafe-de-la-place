...
const modaleQRCode = document.getElementById("qrcode");
...
function makeRow(product) {
  ...
  const btnQRCOde = makeButton("Q", function () {
    const qrcode = new QRCode(modaleQRCode, {
      text: JSON.stringify(this),
      width: 370,
      height: 370,
    });
    modaleQRCode.title = product.nom;
    modaleQRCode.style.display = "block";
    modaleQRCode.addEventListener("click", function () {
      qrcode.clear();
      modaleQRCode.innerHTML = "";
      modaleQRCode.style.display = "none";
    }, { once: true });
  });
  td2.append(btnModify, btnSuppr, btnQRCOde);
